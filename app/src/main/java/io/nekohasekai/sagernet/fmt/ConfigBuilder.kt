...
import io.nekohasekai.sagernet.bg.SkVpnService
...
fun buildConfig(
    ...
    return SingBoxOptions.MyOptions().apply {
        if (!forTest && DataStore.enableClashAPI) experimental = SingBoxOptions.ExperimentalOptions().apply {
        ...
        if (!forTest) {
            if (isVPN) inbounds.add(Inbound_TunOptions().apply {
                ...
                when (ipv6Mode) {
                    IPv6Mode.DISABLE -> {
                        inet4_address = listOf(SkVpnService.PRIVATE_VLAN4_CLIENT + "/28")
                    }

                    IPv6Mode.ONLY -> {
                        inet6_address = listOf(SkVpnService.PRIVATE_VLAN6_CLIENT + "/126")
                    }
        ...
        for (rule in extraRules) {
            ...
            val uidList = rule.packages.map {
                if (!isVPN) {
                    Toast.makeText(
                        StarKnight.application,
                        StarKnight.application.getString(R.string.sk_route_need_vpn, rule.displayName()),
                        Toast.LENGTH_SHORT
                    ).show()
                }
        ...
        remoteDns.firstOrNull().let {
            ...
            if (!forTest) dns.servers.add(SingBoxOptions.DNSServerOptions().apply {
