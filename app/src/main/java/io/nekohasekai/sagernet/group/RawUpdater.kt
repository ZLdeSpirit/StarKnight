    ...
    override suspend fun doUpdate(
        ...
        if (link.startsWith("content://")) {
            ...
            proxies = contentText?.let { parseRaw(contentText) }
                ?: error(app.getString(R.string.sk_no_proxies_found_in_subscription))
        } else {
            ...
            proxies = parseRaw(Util.getStringBox(response.contentString))
                ?: error(app.getString(R.string.sk_no_proxies_found))
        ...
        if (text.contains("proxies:")) {
            ...
            try {
                ...
                for (proxy in (yaml["proxies"] as? (List<Map<String, Any?>>) ?: error(
                    app.getString(R.string.sk_no_proxies_found_in_file)
                ))) {
